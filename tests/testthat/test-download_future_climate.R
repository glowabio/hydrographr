...
test_that("1 downloading WITHOUT preexisting futureclimate90m_paths_file_sizes.txt", {
    ...
    # Prepare:
    download_dir <- file.path(tmpdir, "test_download_future_climate_1")
    ...
    # Check:
    created_files <- list.files(file.path(download_dir, 'Climate', '2071_2100/bio1'))
...
test_that("2 downloading WITH preexisting environment90m_paths_file_sizes.txt...", {
    ...
    # Prepare:
    download_dir <- file.path(tmpdir, "test_download_future_climate_2")
    if (!(file.exists(file_size_file))) {
...
test_that("3 unzipping, removing the zips", {
    ...
    # Prepare:
    download_dir <- file.path(tmpdir, "test_download_future_climate_3")
    if (!(file.exists(file_size_file))) {
        download.file(file_size_file_url, destfile = file_size_file, mode = "wb")
    }
...
test_that("4 unzipping without removing the zips", {
    ...
    # Prepare:
    download_dir <- file.path(tmpdir, "test_download_future_climate_4")
    if (!(file.exists(file_size_file))) {
        download.file(file_size_file_url, destfile = file_size_file, mode = "wb")
    }
...
test_that("5: MUST FAIL: Wrong variable", {
    ...
    # Prepare:
    download_dir <- file.path(tmpdir, "test_download_future_climate_5")
    if (!(file.exists(file_size_file))) {
        download.file(file_size_file_url, destfile = file_size_file, mode = "wb")
    }
...
test_that("6: MUST FAIL: Wrong scenario", {
    ...
    # Prepare:
    download_dir <- file.path(tmpdir, "test_download_future_climate_6")
    if (!(file.exists(file_size_file))) {
        download.file(file_size_file_url, destfile = file_size_file, mode = "wb")
    }
...
test_that("7: MUST FAIL: Wrong scenario", {
    ...
    # Prepare:
    download_dir <- file.path(tmpdir, "test_download_future_climate_7")
    if (!(file.exists(file_size_file))) {
        download.file(file_size_file_url, destfile = file_size_file, mode = "wb")
    }
...
test_that("8: MUST FAIL: Wrong scenario", {
    ...
    # Prepare:
    download_dir <- file.path(tmpdir, "test_download_future_climate_8")
    if (!(file.exists(file_size_file))) {
        download.file(file_size_file_url, destfile = file_size_file, mode = "wb")
    }
