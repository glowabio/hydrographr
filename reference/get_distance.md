...
points located in one basin. To calculate the distance along the
network, point coordinates need to be snapped to the stream network
using the function [`snap_to_network()`](snap_to_network.md) or
...
- stream_layer:
  ...
  defined to calculate the distance along the network. (In the case of
  Hydrography90m, the relevant files have the format
...
To calculate the euclidean distance between all pairs of points the
...
points are located in different basins, the function
[`get_distance_parallel()`](get_distance_parallel.md) should be used.
...
- [`get_distance_parallel()`](get_distance_parallel.md) to calculate the
  distance along the network in more than two basins in parallel.
