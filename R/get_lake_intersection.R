...
#' @param lake_name character. The name of the attribute table column in the geo-spatial files containing the lake ids; i.e. "HydroLAKES_polys_v10" for HydroLAKES shapefiles.
#' @param buffer character. users can either set buffer to TRUE, FALSE or provide buffer size in meters;
#' if TRUE a predefined buffer is used where the size of the buffer depends on the lake size; if FALSE no buffer is applied; Default is TRUE
...
get_lake_intersection <- function(data, lake_id = "HydroLAKES_polys_v10", lakes, lake_name, buffer = TRUE, edge, stream, flow, glbasins, glcompunits, cubasins, lake_dat, quiet = TRUE) {
  
  ### remove 0s and NA's from the output data table from extract_lake_ids ###
  ### list lake IDS from extract_lake_ids table in an array with space seperating id to be in the format LK=(1293 14297)
  ...
  ****************************************************************************"
  ...
  writeLines(file_content, paste0(tempdir(), "mspa-parameters.txt"))
