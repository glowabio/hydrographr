...
We will calculate the distance between all point locations.
...
#### 2. Distances from the basin's outlet
...
```{r, eval = F}
spdata_centr <- get_centrality(stream_graph, index = "all", mode = "in")
```

```{r, eval = F, echo = F}
fwrite(spdata_centr, paste0(wdir, "/data/subc_centrality.csv"), 
       sep = ",", row.names = F, quote = F)
```

```{r, echo = F}
# spdata_centr <- fread(paste0(wdir, "/data/subc_centrality.csv")) %>% head()
```
...
We now attach the centrality indices to the species data 
using the subcatchment id
```{r}
# point_locations_snapped <- left_join(point_locations_snapped, spdata_centr)
```

### Extracting zonal statistics of topographic variables

We will calculate the zonal statistics of the Hydrography90m
variables for all the sub-catchments of the study area.
...
# var_layers <- list.files(study_area_dir)
# var_layers
# ```

```{r}
elv_layer <- "elv_study_area_crop.tif"
```
A good practice before aggregating the variables is to check their
NoData values:

```{r, eval = FALSE}
report_no_data(data_dir = study_area_dir, var_layer = elv_layer)
#                    Raster NoData
# 1 elv_study_area_crop.tif  -9999
```
...
system.time(stats_table_zon <- extract_zonal_stat(
          data_dir = study_area_dir,
          subc_layer = paste0(study_area_dir, "/sub_catchment_h18v02_crop.tif"),
          subc_id = "all",
          var_layer = elv_layer,
          out_dir = paste0(wdir, "/data"),
          file_name = "zonal_stats_elv.csv",
          n_cores = 2))
```
