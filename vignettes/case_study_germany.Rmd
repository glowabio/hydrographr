...
We can reclassify the stream segment raster layer based on the centrality value
of each stream, using the function `reclass_raster`. Essentially, we will create 
a new raster file, in which each stream pixel will have a centrality value, 
instead of the unique ID of the stream (or sub-catchment ID). 
...
The values in some of the original raster files were scaled, so we need to
re-scale them before proceeding to any analyses.
