...
to the next dam for the current and future state.
...
The catfish is listed as 'endangered' by the IUCN red list. Wetlands are the 
preferred habitat and it is migrating for spawning.As an migratory species, it 
is mostly threatened by dams along the stream network. Further, it is threatened 
by fishing during the spawning season in the upstream part of the river. 
...
Then we will filter our fish occurrence and dam point locations using the polygon 
of the drainage basin and snap the point locations to the stream network. 
Afterwards, we will reduce our drainage basin using the dam upstream of all fish
occurrence sites as our outlet location to calculate the upstream catchment from 
this point location. In a next step, we will calculate the distances along the 
stream network between each point location (fish and dams) and get the 
up- and downstream stream egments from each point locations as a graph. Afterwards,
we will query the tables and evaluate the shortest distance to the next dam as well
as the number of dams. 

![](../vignettes/data_brazil/data/figures/workflow.png)
...
extent the drainage basin of São Francisco river. We will then use the polygon of 
