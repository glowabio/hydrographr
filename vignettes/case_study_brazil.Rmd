...
We will evaluate the connectivity of the habitats for a catfish 
in Brazil, the current and future state.
...
Afterwards we will download the Hydrography90m layers, crop and merge them, if 
necessary. 


*Conorhynchos conirostris* is a fish species endemic to the São Francisco river 
basin in Brazil. The catfish is listed as 'endangered' by the IUCN 
red list. Wetlands are the preferred habitat and it is migrating for spawning.
As an migratory species, it is mostly threatened by dams along the stream network.
Further, it is threatened by fishing during the spawning season in the upstream part
of the river. 
...
![](../vignettes/data_brazil/data/figures/occurrence_locations.png)
...
unique ID and is the necessary base-layer for the workflow. We will need these
layers for cropping and filtering and to extract the unique IDs.
...
Further, we will download raster layer of the 
...
to calculate the distance between point locations and to create a graph to find
...
files first and than merge them.
...
extent the drainage basin ofSão Francisco river. We will then use the polygon of 
the drainage basin to crop all downloaded raster tiles and then merge them. 
...
### Filtering of the species occurrence and dam locations

The information of present and future dam locations is provided by the GRanD and
the FHRed database, respectively. Both datasets can be downloaded from
[globaldamwatch.org](https://www.globaldamwatch.org/directory).

To filter the point locations of the fish occurrence and dams we will use the
polygon of the drainage basin to only keep the locations which are located within 
the drainage basin. For this step we will use the functions of the `sf` package.  
...
### Snap species occurrence and dam sites to a stream segment within a certain distance and with a certain flow accumulation

Before we can calculate the upstream catchment of a defined point location or
the distance along the stream network between species occurrence and dam sites, 
...
The `hydrographr` package offers to different snapping function the 
`snap_to_network` and the `snap_to_subc_segment`. The first function uses
a defined distance radius and if also defined a flow accumulation threshold, 
while the second function snaps the point location to the stream segment of the 
...
occurrence and dam sites will not be snapped to a headwater stream (first order
stream) also there is a higher order stream next to it. 

![](../vignettes/data_brazil/data/figures/snapping.png)

The function is implemented in a for-loop to start searching for streams with a 
very high flow accumulation of 400,000 km² in a very short distance and than 
slowly decrease the flow accumulation to 100 km². If there are still sites left
which were not snapped to stream segment yet. The distance will increase from 10 
up to 30 cells.
...
sites to the stream network, not matter how much you increase the distance radius.
If this happens the coordinates need to be changed slightly within the same cell
until the point gets snapped. I might happen that this needs to be done multiple
times.
...
# Load the polygon of the drainage basin
basin_poly <- read_sf(paste0(saofra_dir,"/basin_481051.gpkg"))
#stream_vect <- read_sf(paste0(saofra_dir,"/order_vect_segment_481051.gpkg"))
...
![](../vignettes/data_brazil/data/figures/point_location_snapped.png)
...
restricted to the areas upstream of the Sobradinho dam (site ID 2516) and all
our fish occurrence sites are located upstream of the dam as well. Therefore,
...
polygon of the upstream catchment to filter our point locations using the functions
...
# Get all sub-catchment IDs of upstream basin
...
upstream_segment <- read_geopackage(gpkg =  paste0(saofra_dir, 
                                                "/order_vect_segment_481051.gpkg"),
                                    import_as = "sf",
                                    subc_id = subc_ids$sub_catchment_2516,
                                    name = "stream")
  
write_sf(upstream_segment, paste0(upcatch_dir, "/order_vect_segment_2516.gpkg"))
upstream_segment <- read_sf(paste0(upcatch_dir, "/order_vect_segment_2516.gpkg"))                                     
...
![](../vignettes/data_brazil/data/figures/upstream_catchment.png)

### Get distance along the stream network

Toe determine the distance to the closest dam up- and downstream of each
fish occurrence site we will first calculate the distance between all point 
locations using the function `get_distance`.

```{r eval=FALSE}
basin_rast <- paste0(upcatch_dir, "/upstream_basin_2516.tif")
stream_vect <- paste0(upcatch_dir, "/order_vect_segment_2516.gpkg")
...
fish occurrence sites, we need to load the GeoPackage of the stream segments (
order_vect_segment) as a graph. Further we will need the sub-catchment IDs of 
the fish occurrence sites to use the function `get_catchment_graph`. The
function will return for each fish occurrence site all sub-catchment IDs 
...
stream_network_graph <- read_geopackage(gpkg = stream_vect, import_as = "graph")
...
kbl(head(upstream_segments_168036446)) %>%                        
  kable_styling(bootstrap_options = "striped", full_width = F, position = "left") %>%     
  scroll_box(width = "700px", height = "200px")
```
...
the sub-catchment IDs up- and downstream of each fish occurrence site we can 
...
```{r fig.height=10, fig.width = 20, unit = "cm"} 
