---
title: "hydrographr case study 1"
...
knitr::opts_chunk$set(echo = TRUE, warning=FALSE, message=FALSE,  out.width='50%', fig.align='center')
...
Import the map of Cuba to visualise the species occurrences
```{r}
cuba_map <- vect("~/Documents/PhD/data/world_with_islands/cuba_poly/CUB_adm0.shp")
spdata_vect <- vect(spdata, geom=c("longitude", "latitude"))
```

<!-- ```{r plot, echo=TRUE, message=FALSE, warning=FALSE, out.width='80%', fig.align='center'} -->
<!-- plot(cuba_map) -->
<!-- plot(spdata_vect, add = TRUE, pch = 16, cex = 0.5, col = "purple") -->
<!-- ``` -->

```{r}
leaflet::leaflet() %>% 
  leaflet::addPolygons(data = cuba_map, color = "purple") %>% 
  leaflet::addCircles(data = spdata_vect, color = "yellow")
```
