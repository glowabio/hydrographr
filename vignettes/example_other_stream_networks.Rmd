...
Data of the NHDPlus dataset can be downloaded using the R package
...
# We can crop to another bounding box, and then try to merge the two cropped layers
...
set_no_data(data_dir = paste0(rast_dir, "/NHDPLUS_H_2006_HU4_RASTER/HRNHDPlusRasters2006"), var_layer = "cat.tif", no_data = -32768)
...
# We first need to convert the bounding box coordinates to WGS84, that is the coordinate system of the CHELSA Bioclim layers.
...
# Same as before, we need to filter out the no data values to get a better grasp of the resulting table
...
# Create example dataframe with reclassification rules to reclassify the value of all basins containing species data to 1, and all the rest to -99999.
...
# I think that stream IDs don’t correspond to catchment ids, so it’s not possible to filter the .gpkg using the ids of the catchment raster
```
...
flow_line_graph <- read_geopackage(gpkg = paste0(gdb_dir, "/nhdplus_2006-HU4.gpkg"), 
                layer_name = "NHDFlowline",
                import_as = "graph", name = "Permanent_Identifier")

# Import the catchment layer as a spatial vector
catchment_sp <- read_geopackage(gpkg = paste0(gdb_dir, "/nhdplus_2006-HU4.gpkg"), 
