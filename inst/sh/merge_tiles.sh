...
if [[ ${f: -4} == ".tif" ]] 
then
    gdalbuildvrt -overwrite $out/merge.vrt ${farray[@]}
    gdal_translate  -co COMPRESS=DEFLATE -co ZLEVEL=9  $out/merge.vrt $out/${outname}
    rm $out/merge.vrt
else
    ogrmerge.py -single -progress -skipfailures -overwrite_ds -f GPKG \
        -o $out/merge.gpkg ${farray[@]}
    ogr2ogr  -nlt POLYGON -dialect sqlite \
        -sql "SELECT ST_Union(ST_MakeValid(geom)),"ID" FROM merged GROUP BY "ID" " \
        $out/${outname} $out/merge.gpkg 
    rm $out/merge.gpkg
fi
