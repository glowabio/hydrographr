...
October 2025 in Bogotá, Colombia.
...
docker-volume upon login.

### Data inside the container

You can copy data into the container's file system using `docker cp`, but it is
not really that useful.



## Data for the Living Data Conference Workshop

### Read-only data

Make dir on the server running the JupyterHub.

```
mkdir -p /opt/jupyterhub/data_readonly
```

Copy data needed for the analysis there:

```
cd data_readonly/
wget https://something.de/points_fish_colombia.csv
```

Change ownership, so it can be read:

```
sudo chown 1000:100 data_readonly/points_fish_colombia.csv
```

For the workshop at Living Data Conference, we had the following files as
read-only data. (Much of it is too large to include it in this repo on GitHub).

```
ls -1 data_readonly/
| notebook.Rmd
| notebook.ipynb
| points_fish_colombia.csv
| stream_magdalena.gpkg
| centrality_metrics.csv
| redlist_range_Pseudoplatystoma_magdaleniatum.gpkg
| predict_table.csv
| basin_h10v06_tmp.gpkg
| basin_h10v08_tmp.gpkg
| coordinates_zB9sMviu.txt
| coordinates_zIlNX4w0.txt
| r.stream.order
| r.watershed
| chelsa_bioclim_v2_1
...
This data was be bind-mounted as read-only to all the users' Notebooks. They
could use it as input data for the analysis run during the workshop, but not
make/store changes (for changing/storing, it needs to be copied to a read-write
